#!/usr/bin/env python3
"""
Generate round pixel art sun icon for the weather widget app (for adaptive icons).
"""
...
def create_round_sun_icon(size):
    """Create a round pixel art sun icon"""
    block_size = size // 16
    ...
    # Draw rays (same as square icon)
    fill_rect(7 * block_size, block_size, 2 * block_size, block_size, amber)
    fill_rect(3 * block_size, 3 * block_size, block_size, block_size, amber)
    fill_rect(4 * block_size, 4 * block_size, block_size, block_size, amber)
    fill_rect(12 * block_size, 3 * block_size, block_size, block_size, amber)
    fill_rect(11 * block_size, 4 * block_size, block_size, block_size, amber)
    fill_rect(block_size, 7 * block_size, block_size, 2 * block_size, amber)
    fill_rect(14 * block_size, 7 * block_size, block_size, 2 * block_size, amber)
    fill_rect(3 * block_size, 12 * block_size, block_size, block_size, amber)
    fill_rect(4 * block_size, 11 * block_size, block_size, block_size, amber)
    fill_rect(12 * block_size, 12 * block_size, block_size, block_size, amber)
    fill_rect(11 * block_size, 11 * block_size, block_size, block_size, amber)
    fill_rect(7 * block_size, 14 * block_size, 2 * block_size, block_size, amber)
...
for folder, size in sizes.items():
    ...
    png_data = create_round_sun_icon(size)
